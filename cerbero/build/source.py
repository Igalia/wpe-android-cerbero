...
class Source (object):
    ...
    async def cargo_update(self, offline, logfile):
        ...
        await shell.async_call([self.cargo, 'update'] + update_args,
                               cmd_dir=self.src_dir, logfile=logfile,
...
class Tarball(BaseTarball, Source):
    ...
    async def extract_impl(self, fetching=False):
        m.action(_('Extracting tarball to %s') % self.src_dir, logfile=get_logfile(self))
...
class Git (GitCache):
    ...
    async def extract_impl(self, fetching=False):
        ...
        await git.local_checkout(self.src_dir, self.repo_dir, self.commit, logfile=get_logfile(self), use_submodules=self.use_submodules)
...
class GitExtractedTarball(Git):
    ...
    async def extract_impl(self):
        ...
        self._files['.in'] = [x for x in self._files['.in'] if
                os.path.join(self.src_dir, 'm4') not in x]
        self._fix_ts()
...
class LocalDir(Source):
    ...
    def __init__(self):
        ...
        self.src_dir = os.path.abspath(self.path)
